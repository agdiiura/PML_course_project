...
We can reduce the number of columns in the training dataset because the first column contains the indeces. We also remove the columns with at least 60% of NA values and near zero variance variables using the function `nearZeroVar` contained in the `caret` library. Also the variables $1\div6$ can be neglected. To do this we create a mask variable called `mask_training`.
